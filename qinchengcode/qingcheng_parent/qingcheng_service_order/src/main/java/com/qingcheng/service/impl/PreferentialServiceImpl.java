...
public class PreferentialServiceImpl implements PreferentialService {
    ...
    public int findPreMonetByCategoryId(Integer catrgoryId, int money) {
         Example example=new Example(Preferential.class);
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("state","1");//状态
        criteria.andEqualTo("categoryId",catrgoryId);//分类
        criteria.andLessThanOrEqualTo("buyMoney",money);//消费额
        criteria.andGreaterThanOrEqualTo("endTime",new Date());//截止日期
        criteria.andLessThanOrEqualTo("startTime",new Date());//开始日期
