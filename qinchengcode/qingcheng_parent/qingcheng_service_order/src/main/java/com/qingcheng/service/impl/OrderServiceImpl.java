...
public class OrderServiceImpl implements OrderService {
    ...
    public Map<String,Object> add(Order order) {
        //获取选中的后无车
        List<Map<String, Object>> cartList = cartService.findNewOredrItemList(order.getUsername());
        ...
        try {
            rabbitTemplate.convertAndSend("","queue.skuback","这是测试数据");
            //保存订单主表
            ...
            double proption = (double) order.getPayMoney() / totalMoney;

            for (OrderItem orderItem : orderItemList) {
                orderItem.setId(idWorker.nextId()+"");
                orderItem.setOrderId(order.getId());
                orderItem.setPayMoney((int)(orderItem.getMoney()*proption));
...
//            int i=1/0;
    ...
    public void updatePayStatus(String orderId, String transactionId) {
        ...
        if (order!=null&&order.getPayStatus().equals("0")){
            ...
            orderLog.setRemarks("交易流水号:"+transactionId);//交易流水号
