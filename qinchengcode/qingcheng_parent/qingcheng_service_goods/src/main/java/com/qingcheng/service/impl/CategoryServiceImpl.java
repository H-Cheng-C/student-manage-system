...
public class CategoryServiceImpl implements CategoryService {
    ...
    public void add(Category category) {
        ...
        saveCategoryTreeToRedis();//数据修改后从新进行缓存预热
    }
    ...
    public void saveCategoryTreeToRedis() {
        ...
        List<Category> categoriyList = categoryMapper.selectByExample(example);
        List<Map> categoryTree= findByParentId(categoriyList,0);

        //存入redis
        redisTemplate.boundValueOps(CacheKey.CATEGORY_TREE).set(categoryTree);
    }

    private List<Map> findByParentId(List<Category> categoriyList,Integer parentId){
        List<Map> mapList=new ArrayList<Map>();
        for (Category category : categoriyList) {
            if (category.getParentId().equals(parentId)){
                HashMap<Object, Object> map = new HashMap<>();
                map.put("name",category.getName());
                map.put("menus",findByParentId(categoriyList,category.getId()));
                mapList.add(map);
            }
        }
        return mapList;
